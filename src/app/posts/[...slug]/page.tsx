...
export default async function Post({
  ...
  const baseUrl = `${process.env.NEXT_PUBLIC_SITE_URL}${process.env.BASE_PATH}`;
  const postUrl = `${baseUrl}/posts/${slug.join("/")}`;

  return (
    <>
      <PostSidebar posts={allPosts} currentSlug={currentSlug} />

      <article className="max-w-3xl mx-auto px-6 py-20">
        <div className="mb-16">
          <Link
            href="/"
            className="text-sm font-medium text-stone-400 dark:text-stone-600 hover:text-stone-900 dark:hover:text-stone-100 transition-colors mb-12 inline-block"
          >
            &larr; Back to home
          </Link>
        ...
        <div className="mt-32 pt-12 border-t border-stone-100 dark:border-stone-900">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-8">
            <Link
              href="/"
              className="text-sm font-bold text-stone-900 dark:text-stone-100 hover:opacity-70 transition-opacity"
            >
              &larr; Back to home
            </Link>
            ...
            <CopyButton text={postUrl}>Copy link</CopyButton>
            </div>
          </div>
        </div>
      </article>
