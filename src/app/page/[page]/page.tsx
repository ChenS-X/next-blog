    ...
    <div className="max-w-3xl mx-auto px-6 py-20">
      ...
      {totalPages > 1 && (
        <div className="mt-20 pt-12 border-t border-stone-100 dark:border-stone-900 flex items-center justify-between">
          <Link
            href={currentPage > 1 ? (currentPage === 2 ? '/' : `/page/${currentPage - 1}`) : '#'}
            className={`text-sm font-bold transition-opacity ${currentPage > 1 ? 'hover:opacity-50' : 'opacity-20 cursor-not-allowed'}`}
          >
            &larr; Previous
          </Link>
          ...
          <Link
            ...
            className={`text-sm font-bold transition-opacity ${currentPage < totalPages ? 'hover:opacity-50' : 'opacity-20 cursor-not-allowed'}`}
          >
            Next &rarr;
          </Link>
        </div>
